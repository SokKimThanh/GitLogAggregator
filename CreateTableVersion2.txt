...
-- ----------------------------
-- Table: IntermshipDirectories
-- Mục đích: Quản lý thông tin thư mục thực tập theo tuần
-- ----------------------------
CREATE TABLE IntermshipDirectories (
    IntermshipDirectoryId INT PRIMARY KEY IDENTITY(1,1), -- ID tự động tăng, khóa chính
    IntermshipWeekFolder NVARCHAR(MAX) NOT NULL,         -- Tên thư mục tuần thực tập (VD: "Week1")
    IntermshipStartDate DATETIME NOT NULL,               -- Ngày bắt đầu thực tập
    IntermshipEndDate DATETIME NOT NULL,                 -- Ngày kết thúc thực tập
...
-- ----------------------------
CREATE TABLE Weeks (
    WeeklyId INT PRIMARY KEY IDENTITY(1,1),              -- ID tự động tăng, khóa chính
...
CREATE TABLE CommitPeriods (
    ...
    PeriodStartTime DATETIME NOT NULL,                   -- Thời gian bắt đầu buổi
    PeriodEndTime DATETIME NOT NULL,                     -- Thời gian kết thúc buổi
    CreatedAt DATETIME DEFAULT GETDATE(),                -- Thời gian tạo bản ghi
    UpdatedAt DATETIME DEFAULT GETDATE()                 -- Thời gian cập nhật cuối
);
